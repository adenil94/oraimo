...
// j'ai modifié le nom en EditUserInfos
    ...
    public function DelUserInfos($table, $checkby, $valueckeck) {
        $text1 = "UPDATE $table SET status ='supprimer' ,   edit_date= '".$this->getCurentimeAs('dateTime')
."' WHERE $checkby LIKE $valueckeck";
        try {
    ...
    public function ifEntiterExist($table, $checkby, $valucheck, $checkby2 = null, $valucheck2 = null) {
        ...
        if (isset($checkby2) && isset($valucheck2)) {
            $text = "SELECT COUNT(*) FROM " . $table . " WHERE " . $checkby . " LIKE '" . $valucheck . "' AND " . $checkby2 . " LIKE '" . $valucheck2 . "' AND status='créer' ";
        } else {
            $text = "SELECT COUNT(*) FROM " . $table . " WHERE " . $checkby . " LIKE '" . $valucheck . "' AND status='créer' ";
        }
    ...
    public function getEntiter($table) {
        ...
        $text = "SELECT * FROM " . $table . " WHERE  status='créer'";
        try {
            $request = $this->bdd->query($text);
