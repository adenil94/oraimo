...
// j'ai modifié le nom en EditUserInfos
     ...
     * @return true  si tous c'est bien passer et l'exeption si il y a une erreur 
     */
    public function DelUserInfos($table, $checkby, $valueckeck,$checkby2 = null, $valucheck2 = null) {
        if (isset($checkby2) && isset($valucheck2)) {
             $text1 = "DELETE FROM ". $table ." WHERE ". $checkby ." LIKE ".$valueckeck. "  AND " . $checkby2 . " LIKE '" . $valucheck2 . "'";
        }else{
            $text1 = "DELETE FROM $table WHERE $checkby LIKE $valueckeck";
        }
    ...
    /**
     * @author : andil Adebiyi
     * ifEntiterExist() est une methode qui suprime les informations de l'utilisateur dans m'inporte qu'elle table 
     * @param  string : $table  Le nom de la table dans laquelle les informations seront suprimer 
     * @param $checkby le nom du champs de vérification 
     * @param $valueckec valeur correspondant au champs de vérifiaction 
     * @date : 11/10/2019
    ...
    public function ifEntiterExist($table, $checkby, $valucheck, $checkby2 = null, $valucheck2 = null) {
        ...
        if (isset($checkby2) && isset($valucheck2)) {
            ...
            $text = "SELECT COUNT(*) FROM " . $table . " WHERE " . $checkby . " LIKE '" . $valucheck . "'";
        }
    ...
    public function counts($table, $checkby = null, $valcheck = null) {
        ...
        if (isset($checkby) && isset($valcheck)) {
            $text = "SELECT COUNT(*) FROM " . $table . " WHERE " . $checkby . " LIKE '" . $valcheck . "'";
        } else {
     ...
     */
    public function LoginUserInfos($email_numero, $pasw) {

        $table = "user";
        $checkby = 'email';

        if ($this->ifEntiterExist($table, "email", $email_numero, "psw", $pasw)) {

            return $this->getEntiter($table, "email", $email_numero, $manyinfos = "No");
        } else if ($this->ifEntiterExist($table, "numero", $email_numero, "psw", $pasw)) {

            return $this->getEntiter($table, "numeros", $email_numero, $manyinfos = "No");
        } else {
            return false;
        }
    }
    ...
    public function getEntiter($table) {
        ...
        try {
            $request = $this->bdd->query($text);
            $entiter = $request->fetchAll(PDO::FETCH_ASSOC);
            return $entiter;
        } catch (PDOException $e) {
    ...
    public function getEntiterWhitlimit($table, $limite) {
        ...
        $text = "SELECT * FROM " . $table.' LIMIT '.$limite;
        try {
            $request = $this->bdd->query($text);
            $entiter = $request->fetchAll(PDO::FETCH_ASSOC);
            return $entiter;
    ...
    public function getEntiterUnique($table, $checkby, $valcheck) {
        $text = "SELECT * FROM " . $table . " WHERE " . $checkby . " LIKE '" . $valcheck . "'";

        try {
            $request = $this->bdd->query($text);
            $entiter = $request->fetch(PDO::FETCH_ASSOC);
            return $entiter;
        } catch (PDOException $e) {
            echo "Récuperation des informations  échoué:" . $e->getMessage();
            return false;
     ...
     */
        ...
        $test = "SELECT * FROM " . $table . " WHERE " . $checkby . " LIKE '" . $checkvalue . "'";
        if ($orderby)
        ...
        $text="SELECT * FROM $table WHERE $champ1 LIKE '$value1' AND $champ2 LIKE '$value2'";
    
        $request=$this->bdd->query($text);
        $resulta =  $request->fetch(PDO::FETCH_ASSOC);
        $request->closeCursor();
     ...
     public function getCurentimeAs($type){
      ...
      /**
     * @author : andil Adebiyi
     * getModule() est une methode qui renvoie un module 
     * @param  string : $module  Nom de la classe correspondan au module  
     * @date : 29/03/2020
     * @return l'instance de la classe du module
     */


     public function getMicorService($module){

        return new $module($this->bdd);
     }
